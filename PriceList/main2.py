...
class PriceList(MDApp):
    ...
    def __init__(self, **kwargs):
        ...
        self.kv_loaded = set()  # Para trackear archivos .py cargados
    ...
    def cargar_kv(self, nombre_kv):
        if nombre_kv not in self.kv_loaded:
            try:
                modulo = importlib.import_module(f"views.{nombre_kv}")
                variable = getattr(modulo, f"KV_{nombre_kv.upper()}")
                Builder.load_string(variable)
                self.kv_loaded.add(nombre_kv)
            except Exception as e:
                print(f"Error al cargar {nombre_kv}: {e}")
